  ...
  return (
    <section className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
      ...
      <div className="relative min-h-[560px] lg:min-h-[660px]">
        <div className="absolute inset-x-0 top-0 h-[560px] overflow-hidden rounded-[2rem] border border-[#e6d7bf] bg-[#24170f] shadow-[0_34px_110px_rgba(15,23,42,0.22)] lg:h-[640px]">
          {imageSrc ? (
            <img
              src={imageSrc}
              alt="Premium LEGO collection displayed in a lit glass cabinet"
              className="h-full w-full object-cover"
            />
          ) : null}
          <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(5,10,24,0.10)_0%,rgba(5,10,24,0.02)_42%,rgba(5,10,24,0.44)_100%)]" />
          <div className="absolute inset-4 rounded-[1.65rem] border border-white/30 shadow-[inset_0_0_70px_rgba(255,255,255,0.10)]" />
          <div className="absolute left-8 top-8 z-20 rounded-full bg-white px-3 py-1.5 text-xs font-medium text-blue-700 shadow-sm">
            {primarySignal}
          </div>
          <div className="absolute right-8 top-8 z-20 rounded-full bg-slate-950 px-3 py-1.5 text-xs font-medium text-white shadow-sm">
            {secondarySignal}
          </div>
          <div className="absolute bottom-7 left-7 right-7 z-30 rounded-2xl border border-white/25 bg-white/92 p-5 text-slate-950 shadow-[0_20px_70px_rgba(0,0,0,0.22)] backdrop-blur">
