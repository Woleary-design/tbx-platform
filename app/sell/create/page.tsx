...
function starterPricing(draft: Draft): Pricing {
  ...
  const conditionFactor = draft.condition.includes("New") ? 1.2 : draft.condition.includes("Incomplete") ? 0.72 : draft.condition.includes("Loose") ? 0.85 : 1;
  const base = weight > 0 ? weight * 180 * conditionFactor : 850 * conditionFactor;
  const recommended = roundPrice(base);
  return { fast: roundPrice(recommended * 0.9), recommended, premium: roundPrice(recommended * 1.12), low: roundPrice(recommended * 0.82), high: roundPrice(recommended * 1.18), confidence: "Early estimate", evidence: weight > 0 ? `Starter bulk estimate using ${weight} kg and selected condition` : "Starter estimate until an exact Atlas match is available" };
}
...
export default function CreateListingPage() {
  ...
  useEffect(() => {
    ...
    if (!setNumber) { setPricing(starterPricing(draft)); return; }
    const controller = new AbortController();
    void (async () => {
      setPricingLoading(true);
      try {
        const response = await fetch(`/api/value/${setNumber}?condition=${encodeURIComponent(draft.condition)}`, { signal: controller.signal });
        ...
        setPricing({ fast: roundPrice(low), recommended: roundPrice(recommended), premium: roundPrice(high), low: roundPrice(Math.min(low, recommended * 0.88)), high: roundPrice(Math.max(high, recommended * 1.12)), confidence: Number(quote?.confidence ?? 0) >= 0.75 ? "High" : "Medium", evidence: `${Number(quote?.verified_sales ?? 0)} verified sales · ${Number(quote?.active_listings ?? market?.activeListingCount ?? 0)} active listings` });
      } catch { if (!controller.signal.aborted) setPricing(starterPricing(draft)); }
      finally { if (!controller.signal.aborted) setPricingLoading(false); }
    })();
    return () => controller.abort();
  }, [step, draft.title, draft.condition]);
  ...
  const marketPosition = useMemo(() => {
    ...
    return Math.max(0, Math.min(100, ((price - pricing.low) / Math.max(1, pricing.high - pricing.low)) * 100));
  }, [price, pricing]);
  ...
  return (
    <main className="min-h-screen bg-[#050912] text-white">
      <header className="border-b border-white/[0.06]"><div className="mx-auto flex h-20 max-w-[1200px] items-center justify-between px-5 lg:px-10"><Link href="/" className="flex items-center gap-3 font-black"><span className="grid h-10 w-10 place-items-center rounded-xl border border-[#e8c86a]/25 bg-[#09101d] text-[#e8c86a]"><Boxes className="h-5 w-5" /></span>TBX</Link><Link href="/value" className="inline-flex items-center gap-2 text-sm font-bold text-white/55"><ArrowLeft className="h-4 w-4" /> Back to Value</Link></div></header>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="mb-6 mt-6 block"><span className="text-sm font-bold text-white/70">{label}</span>{children}</label>; }
