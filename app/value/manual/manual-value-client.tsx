...
const flowCopy: Record<IntakeType, { eyebrow: string; title: string; description: string }> = {
  mixed: {
    ...
    description: "A rough description is enough. Atlas only asks for details that can improve the result.",
  },
  minifigures: {
    eyebrow: "Value · Minifigures",
    title: "Tell Atlas about your minifigure",
    description: "Do not know the character or code? That is fine. Describe what you can see and Atlas can work from there.",
  },
  instructions: {
    eyebrow: "Value · Instructions & boxes",
    title: "Tell Atlas what you have",
    description: "Describe the manuals, boxes or packaging. Set details are optional if you do not know them.",
  },
  unknown: {
    eyebrow: "Value · Not sure",
    title: "Let Atlas narrow it down",
    description: "Choose the closest description. You do not need a catalogue match, set number or photo to continue.",
  },
};
...
function choiceClass(active: boolean) {
  return `rounded-2xl border px-4 py-3 text-left text-sm font-bold transition ${active
        ...
        <div className="relative mx-auto max-w-[820px] px-5 py-14 sm:py-20 lg:px-10">
          {submitted ? (
            <div className="rounded-[2rem] border border-white/[0.09] bg-[#09111f]/95 p-8 text-center shadow-[0_40px_120px_rgba(0,0,0,0.48)] sm:p-12">
              ...
              <p className="mx-auto mt-3 max-w-md leading-7 text-white/45">Your details are ready for the next TBX action.</p>
              <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
                <button type="button" onClick={() => setSubmitted(false)} className="inline-flex h-12 items-center justify-center rounded-xl border border-white/12 px-5 font-bold text-white/75">Edit</button>
                <Link href={savedIntent === "Add it to my Collection" ? "/collection" : "/value"} className="inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-[#e8c86a] px-5 font-bold text-[#050912]">Continue <ArrowRight className="h-4 w-4" /></Link>
              </div>
            </div>
          ) : (
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-7 rounded-[2rem] border border-white/[0.09] bg-[#09111f]/95 p-5 shadow-[0_40px_120px_rgba(0,0,0,0.48)] sm:p-8">
                ...
                {flow === "minifigures" && <section><p className="text-sm font-bold text-white/72">Is it one minifigure or several?</p><div className="mt-3 grid gap-3 sm:grid-cols-2">{["Single minifigure", "Minifigure collection"].map((option) => <button key={option} type="button" onClick={() => setItemType(option)} className={choiceClass(itemType === option)}>{option === "Single minifigure" ? "One minifigure" : "A collection"}</button>)}</div></section>}

                {flow === "mixed" && <section><p className="text-sm font-bold text-white/72">Which is closest?</p><div className="mt-3 grid gap-3 sm:grid-cols-2">{["Mixed box of LEGO", "Loose bricks and parts", "Incomplete or unknown sets", "Other LEGO collection"].map((option) => <button key={option} type="button" onClick={() => setItemType(option)} className={choiceClass(itemType === option)}>{option}</button>)}</div></section>}

                {flow === "instructions" && <section><p className="text-sm font-bold text-white/72">What do you have?</p><div className="mt-3 grid gap-3 sm:grid-cols-3">{["Instructions", "Boxes or packaging", "Instructions and boxes"].map((option) => <button key={option} type="button" onClick={() => setItemType(option)} className={choiceClass(itemType === option)}>{option}</button>)}</div></section>}

                {flow === "unknown" && <label className="block"><span className="text-sm font-bold text-white/72">What sounds closest?</span><select value={itemType} onChange={(event) => setItemType(event.target.value)} className="mt-2 h-13 w-full rounded-xl border border-white/10 bg-[#050912] px-4 text-white">{unknownTypes.map((item) => <option key={item}>{item}</option>)}</select></label>}
                ...
                {isMinifigure ? (
                  ...
                  <section className="space-y-5 border-t border-white/[0.07] pt-6">
                    {(flow === "mixed" || flow === "unknown") && <label className="block"><span className="flex items-center gap-2 text-sm font-bold text-white/70"><Scale className="h-4 w-4 text-[#e8c86a]" /> Rough quantity or weight <span className="font-normal text-white/30">Optional</span></span><div className="mt-2 flex"><input name="weight" type="number" min="0" step="0.1" placeholder="e.g. 8" className="h-13 min-w-0 flex-1 rounded-l-xl border border-white/10 bg-[#050912] px-4 text-white" /><select name="weightUnit" className="h-13 rounded-r-xl border border-l-0 border-white/10 bg-[#050912] px-3 text-white"><option>kg</option><option>g</option></select></div></label>}
                ...
                {error && <p role="alert" className="rounded-xl border border-red-400/20 bg-red-400/[0.08] px-4 py-3 text-sm text-red-200">{error}</p>}
                <button type="submit" disabled={saving} className="inline-flex h-14 w-full items-center justify-center gap-3 rounded-2xl bg-[#e8c86a] px-7 font-black text-[#050912] transition hover:bg-[#f1d478] disabled:opacity-60">{saving && <Loader2 className="h-5 w-5 animate-spin" />}{saving ? "Working…" : "Continue with Atlas"}{!saving && <ArrowRight className="h-5 w-5" />}</button>
                <p className="text-center text-xs leading-5 text-white/30">No photo is required to start. Atlas should ask for one later only when it genuinely improves identification or is needed for a Marketplace listing.</p>
              </form>
            </>
          )}
        </div>
