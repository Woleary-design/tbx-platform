...
const steps = [
  { label: "Describe", text: "Choose a known set or describe loose LEGO, spare parts or a mixed box.", icon: PackageOpen },
  { label: "Classify", text: "TBX separates catalogue items from bulk or unlisted material.", icon: ListSearch },
  { label: "Value", text: "Atlas uses the best available evidence for that type of item.", icon: CircleDollarSign },
  { label: "Choose", text: "Sell quickly, list it, or keep it in your collection.", icon: Store },
];
...
const choices = [
  ...
  { title: "Keep my collection", text: "Save known sets and bulk LEGO together in one collection.", icon: Heart },
];
  ...
  return (
    <main className="min-h-screen bg-[#050912] text-white">
        ...
        <div className="relative mx-auto grid min-h-[720px] max-w-[1280px] items-center gap-14 px-5 py-20 lg:grid-cols-[0.82fr_1.18fr] lg:px-10">
          <div>
            ...
            <p className="mt-7 max-w-xl text-lg leading-8 text-white/52">
              Search for a known LEGO set, or describe a mixed box, loose bricks, minifigures or spare parts. You do not need a set number to begin.
            </p>
            <div className="mt-9 flex flex-col gap-3 sm:flex-row">
              <a href="#live-valuation" className="inline-flex h-14 items-center justify-center gap-3 rounded-2xl bg-[#e8c86a] px-7 font-bold text-[#050912] transition hover:-translate-y-0.5 hover:bg-[#f1d478]">
                Value a known set <ArrowRight className="h-5 w-5" />
              </a>
              <Link href="/snap/manual" className="inline-flex h-14 items-center justify-center gap-3 rounded-2xl border border-white/12 bg-white/[0.035] px-7 font-bold text-white/78 transition hover:border-[#e8c86a]/25 hover:text-white">
                I have loose or mixed LEGO <Scale className="h-5 w-5" />
              </Link>
            </div>
            <div className="mt-8 space-y-3 text-sm text-white/48">
              {["No photos required", "No set number required", "Known sets and bulk LEGO follow different valuation paths"].map((item) => (
          ...
          <div id="live-valuation" className="scroll-mt-24 rounded-[2rem] border border-white/[0.09] bg-[#09111f]/95 p-4 shadow-[0_40px_120px_rgba(0,0,0,0.48)] sm:p-6">
            <div className="flex items-center justify-between border-b border-white/[0.07] px-2 pb-5">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#e8c86a]">Known LEGO set</p>
                <p className="mt-1 text-sm text-white/42">Search the Atlas catalogue for current pricing</p>
              </div>
            ...
            <div className="mt-5 rounded-[1.5rem] bg-white p-4 text-slate-950 sm:p-6"><QuickAddSetForm intent="sell" /></div>
            <div className="mt-4 rounded-2xl border border-white/[0.07] bg-white/[0.025] p-4">
              <p className="text-sm font-bold text-white/75">Can&apos;t find the set?</p>
              <p className="mt-1 text-xs leading-5 text-white/38">Continue without choosing a catalogue record. Describe the contents, approximate weight and condition instead.</p>
              <Link href="/snap/manual" className="mt-3 inline-flex items-center gap-2 text-sm font-bold text-[#e8c86a]">Describe loose or unlisted LEGO <ArrowRight className="h-4 w-4" /></Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[1280px] px-5 py-24 lg:px-10">
        ...
        <h2 className="mt-4 max-w-3xl text-5xl font-black tracking-[-0.055em]">Two starting points. One clear next step.</h2>
      ...
      <section className="border-y border-white/[0.06] bg-white/[0.015]">
        <div className="mx-auto max-w-[1280px] px-5 py-24 lg:px-10">
          ...
          <Link href="/snap/manual" className="mt-12 inline-flex h-14 items-center gap-3 rounded-2xl bg-[#e8c86a] px-7 font-bold text-[#050912]">Describe what you have <ArrowRight className="h-5 w-5" /></Link>
        </div>
      </section>
    </main>
  );
